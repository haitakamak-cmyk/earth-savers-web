...
PII scrub は `email|password|token|secret|authorization|cookie|stripe|customer|session|phone|name|address` を含むキーをマスク対象とする。`plan_code` など非機密の業務識別子は保持する。

Phase 2 課題: Production で `/api/internal/sentry-test` を常時有効化する場合は、`SENTRY_ENABLE_TEST_ROUTE=1` だけでなく `x-internal-token` などの追加認証を必須にする。
