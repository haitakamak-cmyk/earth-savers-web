...
export const POLICIES: readonly PolicyEntry[] = [
  {
    ...
    summary:
      "田んぼが消えると、失われるのは米だけではない。雨水を受け止め、景観を保ち、豪雨や災害のときに地域の負担を和らげる側の基盤も、いっしょに消える。だから本提言の芯は、農産物の買取だけの所得保障ではない。農地を食・水・生態系・防災の国土インフラとして捉え、その公共機能を支える仕事へ公が対価を払うことである。市区町村では、給食や備蓄など必要量を安定した価格で毎年調達する層と、草刈り・水路管理・冬期湛水などインフラ管理への対価の層を重ねる。販売収入にこの二層が加わることで、農業を続けられる人を増やす。設計の単位は流域、測る軸は100年とする。",
