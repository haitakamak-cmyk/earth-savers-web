...
- **政策提言ハブ** `/policy`：**`POLICIES` に1件以上ある kind だけ**カード表示（`policyKindsWithPublicEntries()`、`POLICY_HUB_KIND_ORDER` で順序固定）。掲載ゼロの **`/policy/national` 等は `/policy` にリダイレクト**（`PolicyKindPageShell` 先頭）。**サイトマップ**：`/policy` と `policyKindsWithPublicEntries()` に対応する `/policy/{segment}` のみ。関連リンク `policyHubItemsForKinds` は、その kind に掲載が無いときリンクを出さない。
